    ...
    for x in range(81):
        ...
        if (int(grille[x]) == 0):
            ...
            if (len(good_values) == 2):
                grille[x] = str(good_values[index])
            else:
                return -1
    return grille
...
def solver(M):
    ...
    for x in range(81):
        if (M[x] == '0'):
            good_values = []
            for t in range(1, 10):
                if (check_lines(M,x,t) == 1 and check_squares(M, getSquareIndex(x), t) == 1):
                    good_values.append(t)
            #printgrilled(M)
            if(len(good_values) == 1):
                ...
                print("OK")
                return(solver(M))
            x = checkWhatLeftInEachSquare(M)
            if x != 0:
                return solver(x)
            x = checkWhatLeftInEachLine(M)
            if x != 0:
                return solver(x)
            temp = guess(M, 0)
            if temp != -1:
